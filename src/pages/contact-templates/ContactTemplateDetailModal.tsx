...
function MotiveLabel() {
  const motive = useRecordContext();
  return <span>{(motive?.label as string) ?? ""}</span>;
}

export function ContactTemplateDetailModal() {
  ...
  const onClose = () => navigate("/contact-templates");
          ...
          <>
            <ConfirmActionButton
              ...
              title={translate("shared.actions.delete_confirm_title", {
                name: translate("resources.contact-templates.name"),
                _: "Delete",
              })}
            ...
            />
            <Link
              to={`/contact-templates/edit/${record.id}`}
      ...
      {record && (
        <RecordContextProvider value={record}>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            >
              {record.motiveId ? (
                <ReferenceField source="motiveId" reference="contact-motives">
                  <MotiveLabel />
                </ReferenceField>
              ) : (
